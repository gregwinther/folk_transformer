...
flags.DEFINE_string("output_dir", None, "Midi output directory.")

# Specify path to model
flags.DEFINE_string("model_path", None, "Pre-trained model path.")
...
flags.DEFINE_integer(
    "decode_length", 1024, "Length of decode result."
)


class PianoPerformanceLanguageModelProblem(
    score2perf.Score2PerfProblem
):
    @property
...
def generate(
    ...
    base_name = os.path.join(
        FLAGS.output_dir, f"unconditioned_{date_and_time:s}.mid"
    )
    ...
    midi_filename = decode(
        sample_ids, encoder=unconditional_encoders["targets"]
    )
    unconditional_note_seqs = mm.midi_file_to_note_sequence(
        midi_filename
    )
    ...
    mm.sequence_proto_to_midi_file(
        coninuation_note_sequence, base_name
    )
...
def run():
    ...
    if FLAGS.primer_path is None:
        ...
        targets = unconditional_encoders[
            "targets"
        ].encode_note_sequence(primer_note_sequence)
